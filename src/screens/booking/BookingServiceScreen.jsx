...
export default function BookingServiceScreen({ navigation, route = {} }) {
  ...
  const [services, setServices] = useState(() => {
    if (rewardId && incomingServiceId) return [{ id: incomingServiceId }];
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      if (bookingId) {
        ...
        finalTotalPrice = parseFloat(latestRes.data.total_price) || 0;

        // compute only if you have services with price
        originalPriceForDisplay = services.reduce(
        ...
        finalTotalPrice = parseFloat(finalBookingData.total_price) || 0;

        const selectedServicesWithPrice =
          finalBookingData.selectedServices?.length > 0
            ? finalBookingData.selectedServices
            : services.map((s) => ({
              ...s,
              price: parseFloat(s.price || 0),
            }));

        originalPriceForDisplay = selectedServicesWithPrice.reduce(
          (sum, s) => sum + (s.price || 0),
          0
        );

        // If reward was applied, call the redeem endpoint to get the updated pricing
        if (rewardId && finalBookingData?.id) {
          try {
            console.log("🎁 Applying reward to booking...");
            const redeemRes = await axios.post(
              `${BASE_URL}/api/rewards/redeem/`,
              { reward_id: rewardId, booking_id: finalBookingData.id },
              { headers: { Authorization: `Bearer ${token}` } }
            );
            console.log("✅ Reward redeem response:", redeemRes.data);

            // Update pricing from redeem response
            if (redeemRes.data?.original_price !== undefined) {
              originalPriceForDisplay = parseFloat(redeemRes.data.original_price) || originalPriceForDisplay;
            }
            if (redeemRes.data?.discounted_price !== undefined) {
              finalTotalPrice = parseFloat(redeemRes.data.discounted_price) || 0;
            }
            if (redeemRes.data?.booking?.total_price !== undefined) {
              finalTotalPrice = parseFloat(redeemRes.data.booking.total_price) || 0;
            }
            // If the entire booking object is returned, use its pricing
            if (redeemRes.data?.booking) {
              finalBookingData = { ...finalBookingData, ...redeemRes.data.booking };
            }
          } catch (redeemErr) {
            console.log("⚠️ Reward redeem failed:", redeemErr.response?.data || redeemErr);
            // Non-blocking: reward deduction already happened, but pricing update failed
            const redeemMsg = redeemErr.response?.data?.detail || redeemErr.response?.data?.message;
            if (redeemMsg) {
              console.log("Redeem error message:", redeemMsg);
            }
          }
        }
      }
      ...
      if (rewardId) {
        ...
        // Use the originalPriceForDisplay calculated above (from services or finalBookingData)
        // If it's still 0, recalculate from selectedServices
        if (originalPriceForDisplay === 0) {
          const selectedServicesWithPrice =
            finalBookingData.selectedServices?.length > 0
              ? finalBookingData.selectedServices
              : services.map((s) => ({
        ...
        const finalPrice = parseFloat(finalTotalPrice) || 0;
        const originalPrice = parseFloat(originalPriceForDisplay) || 0;

        // For free rewards (finalTotalPrice === 0), show special message
        if (route.params?.rewardType === "free" && finalPrice === 0) {
          message = `🎉 ${rewardTitle} booked!\n\n✨ Service is 100% FREE!\n\nYou saved: ₹${originalPrice.toFixed(2)}\n\nFinal Price: ₹${finalPrice.toFixed(2)}`;
        } else if (finalPrice === 0) {
          // Just in case it's free for another reason
          message = `${rewardTitle} booked!\n🎉 Service is FREE!\n\nFinal Price: ₹${finalPrice.toFixed(2)}`;
        } else {
    ...
    <View style={styles.container}>
      ...
      >
        <ServiceSelection
          ...
          prefillServiceName={route.params?.prefillServiceName}
